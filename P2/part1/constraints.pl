...
are_all_constraints_valid :- not(primary_key_null), not(primary_key_not_unique), not(node_type_not_identified), not(transition_type_not_identified), not(transition_starting_from_stop), not(not_enough_endnodes), not(multiple_start_nodes), not(multiple_end_nodes).
